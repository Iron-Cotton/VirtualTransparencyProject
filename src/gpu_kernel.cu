...
void runReconstructionKernel(
    ...
    unsigned int* d_r, unsigned int* d_g, unsigned int* d_b, unsigned int* d_cnt, // ボクセルバッファ
    uchar4* d_output, 
    ...
    // 4. 可視化 (Visualization)
    // Z=1.0m は nz = floor(12.0 * (1/1.0) + 0.5) = 12 になります
    int targetSlice = 12;
     
    dim3 visBlock(16, 16);
    dim3 visGrid((width + visBlock.x - 1) / visBlock.x, (height + visBlock.y - 1) / visBlock.y);

    sliceVisualizationKernel<<<visGrid, visBlock>>>(
        d_r, d_g, d_b, d_cnt,
        targetSlice,
        width, height,
        d_output
